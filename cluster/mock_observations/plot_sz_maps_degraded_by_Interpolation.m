...
for k=1:length(SZmap)
    ...
    xe=xx(end)+0.5*cl;
    dx=(xe-xb)/degScale;
    xxD=xb+0.5*dx:dx:xe-0.5*dx;
    [ygD,xgD]=meshgrid(xxD,xxD);
    
    
    vvv=reshape(projXY0,[numel(projXY0) 1]);
    projInt=scatteredInterpolant(xxx,yyy,vvv);
    projXY=projInt(xgD,ygD);
    
    vvv=reshape(projYZ0,[numel(projYZ0) 1]);
    projInt=scatteredInterpolant(xxx,yyy,vvv);
    projYZ=projInt(xgD,ygD);
    
    vvv=reshape(projXZ0,[numel(projXZ0) 1]);
    projInt=scatteredInterpolant(xxx,yyy,vvv);
    projXZ=projInt(xgD,ygD);
    ...
    figure
    imagesc(xxD*hub,xxD*hub,log10(projXY'));
    ...
    figure
    imagesc(xxD*hub,xxD*hub,log10(projYZ));
    set(gca,'Ydir','Normal','Fontsize',14)
    ...
    figure
    imagesc(xxD*hub,xxD*hub,log10(projXZ'));
    set(gca,'Ydir','Normal','Fontsize',14)
    bar=colorbar;set(bar,'Fontsize',12);barTitle(bar,'$\log(SZ)\,[\mathrm{kpc^{-2}}]$');
    draw_circle_boxx(gcf,r200,'white');      % draw rvir
    draw_circle_boxx(gcf,r500,'white');      % draw rvir
    ...
    head=sprintf('## bottom/left (Mpc): %s / %s , n_elements: %i x %i , cellSize: %s kpc, units: kpc^-2',...
        num2str(xxD(1)),num2str(xxD(1)),degScale,degScale,num2str(1000*dx));
    global HALO_PATH
