...
if setupFlag
    ...
    losSigma=v200c.*sigmaFac;
    deltaZ=losSigma./(Units.cspeed./Units.km);
    losLength=comoving_distance(deltaZ,'cosmo',cosmoStruct).*1000; % in kpc 
...
for j=1:length(centralMask)
    ...
    for k=1:3 % generate for each cartesian direction
        ...
        losLengthMask=abs(zlen)<=0.5.*losLength(hostInd);
...
%         velMask=abs((vhub+vlos))<sigma(j);
        ...
        satMask=sampleMask & distMask & losLengthMask;
        satMask(j)=false;
...
catName=[DEFAULT_MATFILE_DIR '/SatSample_zredCut_sig1_' simDisplayName];
