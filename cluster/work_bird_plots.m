...
hold on
imagesc(2:9,7:18,log10(result_map));colormap(hot);
hold on
...
hold on
streamslice(xx,yy,velo_x,velo_y,4)
xlim([2 9]);ylim([7 18]);
title(sprintf('Parametric diagram (%dMpc cube)',MPSec))
xlabel('log T');
ylabel('log \rho_g');
%colorbar;
%saveas(gcf,getresultsdir(sprintf('parametric %dMpc.png',MPSec)))
