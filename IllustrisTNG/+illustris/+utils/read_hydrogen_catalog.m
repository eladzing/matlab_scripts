function res=read_hydrogen_catalog(snap,varargin )
...
if ~ischar(snap)
        snapChar=num2str(snap,'%03d');
end



%% defaults
fullPath=[POSTPROCESSING '/hydrogen/gas_' snapChar '.hdf5'];
...
for i=1:len
    ...
    outputStruct.(name)=h5read(fullPath ,['/' name]);
end
...
elseif len>1
    ...
    error('read_hydrogen_catalog - strange value ofr mo. of datasets in file: %d',len)
