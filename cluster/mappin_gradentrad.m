...
slice=mk_slice(gradsrad,ro,slind);
...
for projection = 1:3
  ...
  colormap(avijet);
  bar=colorbar;
  set(get(bar,'Title'),'String','$\nabla_r \mathrm{S/S_{vir}}','Fontsize',12);

  title(sprintf('%s Radial Entropy Gradient, Thickness=%s Mpc/h',clustername,num2str(thick,3)),'Fontsize',12);
