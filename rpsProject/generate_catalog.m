...
%     eps(i)=vmax/sqrt(Units.GG*ms(i)/res.rd(i));
%     
%     %Q Toomre
%     Q=QToomre(res.rr(i,ind),res.vc(i,ind),ms(i),res.rd(i),1,zeta(i),Units.GG);
%     qmin(i)=min(Q);
%     subInd=find(Q<1);
%     if ~isempty(subInd)
...
global DEFAULT_MATFILE_DIR
    ...
    tag='95_10';
    fname=sprintf('galaxy_catalog_%s.mat',tag);
    save([DEFAULT_MATFILE_DIR '/' fname],'cata','-v7.3')
