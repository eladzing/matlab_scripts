...
ss_in=log10(ss_in(:)./snorm);
ss_out=log10(ss_out(:)./snorm);
val_in=val_in(:)./mnorm;
...
for i=1:size(ss_in,1)   %.*size(lss,2).*size(lss,3))
    indx=ceil((ss_in(i)-mnt)./(mxt-mnt).*len);
    if(indx>=1 && indx<=len)
        sh_in(indx,1)=sh_in(indx,1)+val_in(i);
        sh_in(indx,2)=sh_in(indx,2)+1;
    end
end
clear indx val_in ss_in

sh_out=zeros(len,2);
%indx=ceil(((ss_out-mnt)./(mxt-mnt)).*len)
for i=1:size(ss_out,1)   %.*size(lss,2).*size(lss,3))
    indx=ceil((ss_out(i)-mnt)./(mxt-mnt).*len);
    if(indx>=1 && indx<=len)
        sh_out(indx,1)=sh_out(indx,1)+val_out(i);
        sh_out(indx,2)=sh_out(indx,2)+1;
    end
end

clear indx val_out ss_out
...
dt=(mxt-mnt)/len;
tax=mnt:dt:mxt-dt;
figure;plot(tax,[sh_in(:,1)/dt sh_out(:,1)/dt]'); grid; xlim([mnt mxt]);
...
if strcmp(pflag,'print')
  ...
  saveas(gcf,sprintf('%s/%s_shist_%s.png',result_dir,clustername,printag));
end
