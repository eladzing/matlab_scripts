...
%printoutdir='C:/Users/owner/Ubuntu One/cluster/printout/profiles';
titletag='%s %s, $z=%3.1g$';
...
for id=1:length(list)
    ...
    yl=cat(1,ylim,ylim);
    hold on
     semilogx(ll./rv,yl','--r','linewidth',1.5)
     semilogx(ll2./rv,yl','--b','linewidth',1.5)
    hold off
    grid
    ylabelmine('$S\, [\mathrm{Kev\,cm^2}]$')
    titlemine(sprintf(titletag,CLUSTER,'Entropy \& Gradient',zred));
    set(gca,'Fontsize',12)
    
    subplot(2,1,2)
    semilogx(ddr./rv,dsdr,'-','linewidth',2);
    hold on
    semilogx(ddr(f)./rv,dsdr(f),'.r','markersize',25);
    semilogx(ddr(ff)./rv,dsdr(ff),'.b','markersize',25);
    hold off
    xlim(xl);
    grid
    ylabelmine('$dS/dr$')
    xlabelmine('$r/R_{vir}$')
    set(gca,'Fontsize',12)
    %exportfig(gcf,sprintf(name,'shockedge_cledge','png'),'format','png');
    %exportfig(gcf,sprintf(name,'shockedge_cledge','eps'))
