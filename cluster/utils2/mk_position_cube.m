function [xPos,yPos,zPos]=mk_position_cube(boxx,varargin)
    ...
    switch varargin{i}
        ...
        case {'ncell','size'}
            ...
            elseif all(size(s0)==[1 1])
                ...
                error('mk_position_cube: Illegal size argument ')
            end
        ...
        case {'hub','h'}
            ...
            hubl=varargin{i};
        otherwise
            error('mk_position_cube - Illegal argument: %s',varargin{i})
    end
    i=i+1;
end
...
xPos = meshX * ((boxx/hubl)./sz(1));
yPos = meshY * ((boxx/hubl)./sz(2));
zPos = meshZ * ((boxx/hubl)./sz(3));
