...
while i<=length(varargin)
    switch lower(varargin{i})
        ...
        case{'nolog','linear'}
            logFlag=false;
        case{'energy','energy range'} %thicknes of slice in Mpc/h comoving (like box)
            i=i+1;
            thick=varargin{i};
        case {'yz','zy'}
            plotproj(1)=1;
...
slice=mk_slice_xrayProj(cube,slind,slType);
if logFlag
    slice=log10(slice);
end
