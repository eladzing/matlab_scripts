function[fillhandle,msg]=shade2curves(xpoints,upper,lower,color,edge,transparency,add)
%USAGE: [fillhandle,msg]=shade2curves(xpoints,upper,lower,color,edge,add,transparency)
...
%     [ph,msg]=shade2curves(x,a,b,rand(1,3),rand(1,3),0,rand(1,1))
%     grid on
%     legend('Datr')

if nargin<7;add=true;end %default is to add to current plot
...
if length(upper)==length(lower) && length(lower)==length(xpoints)
    ...
    if iscolumn(xpoints);xpoints = xpoints';end
    if iscolumn(upper); upper = upper';end
    if iscolumn(lower); lower = lower';end
