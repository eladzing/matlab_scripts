...
if perlimFlag
    ...
    baseName='qFrac_Ms100_%s_host%s_fgs%s.mat';
    ...
    for i=1:length(hostTags)
        ...
        for j=1:length(tags)
            ...
            if j==1
                ...
                qfHost(i).qfMs1=qfHost(i).(sprintf('qf%s',tag2{j})).ms1Pass.qf;
                qfHost(i).cntMs1=qfHost(i).(sprintf('qf%s',tag2{j})).ms1Pass.cnt;
                
                qfHost(i).qfMsF=qfHost(i).(sprintf('qf%s',tag2{j})).msFull.qf;
                qfHost(i).cntMsF=qfHost(i).(sprintf('qf%s',tag2{j})).msFull.cnt;
                
                qfHost(i).qfMs1P=qfHost(i).(sprintf('qf%s',tag2{j})).ms1Pass.qfProj;
                qfHost(i).cntMs1P=qfHost(i).(sprintf('qf%s',tag2{j})).ms1Pass.cntProj;
                
                qfHost(i).qfMsFP=qfHost(i).(sprintf('qf%s',tag2{j})).msFull.qfProj;
                qfHost(i).cntMsFP=qfHost(i).(sprintf('qf%s',tag2{j})).msFull.cntProj;
                
                qfHost(i).qfMrFP =qfHost(i).(sprintf('qf%s',tag2{j})).massRatSFull.qfProj;
                qfHost(i).cntMrFP=qfHost(i).(sprintf('qf%s',tag2{j})).massRatSFull.cntProj;
                               
                
                
                qfHost(i).bcen=qfHost(i).(sprintf('qf%s',tag2{j})).msFull.bCen-0.25;
            else
                
                if ~isempty(qfHost(i).(sprintf('qf%s',tag2{j})))
                    
                    qfHost(i).qfMs1 = qfHost(i).qfMs1 + qfHost(i).(sprintf('qf%s',tag2{j})).ms1Pass.qf;
                    qfHost(i).cntMs1= qfHost(i).cntMs1+ qfHost(i).(sprintf('qf%s',tag2{j})).ms1Pass.cnt;
                    
                    qfHost(i).qfMsF = qfHost(i).qfMsF + qfHost(i).(sprintf('qf%s',tag2{j})).msFull.qf;
                    qfHost(i).cntMsF= qfHost(i).cntMsF+ qfHost(i).(sprintf('qf%s',tag2{j})).msFull.cnt;
                    
                    qfHost(i).qfMs1P = qfHost(i).qfMs1P + qfHost(i).(sprintf('qf%s',tag2{j})).ms1Pass.qfProj;
                    qfHost(i).cntMs1P= qfHost(i).cntMs1P+ qfHost(i).(sprintf('qf%s',tag2{j})).ms1Pass.cntProj;
                    
                    qfHost(i).qfMsFP = qfHost(i).qfMsFP + qfHost(i).(sprintf('qf%s',tag2{j})).msFull.qfProj;
                    qfHost(i).cntMsFP= qfHost(i).cntMsFP+ qfHost(i).(sprintf('qf%s',tag2{j})).msFull.cntProj;
                    
                    qfHost(i).qfMrFP =  qfHost(i).qfMrFP  + qfHost(i).(sprintf('qf%s',tag2{j})).massRatSFull.qfProj;
                    qfHost(i).cntMrFP=  qfHost(i).cntMrFP + qfHost(i).(sprintf('qf%s',tag2{j})).massRatSFull.cntProj;
...
%             h(hcnt)=hh(1);
            hcnt=hcnt+1;
            yax=[qfHost(k).qff(ind) qfHost(k).qff(ind(end))];
            
        h(hcnt)=stairs(xax([ind ind(end)+1]),yax,'-','color',cmap(k,:),...
            'DisplayName',ltag,'linewidth',1.5);
...
%         
    ...
    %
    %     h(end+1)=plot(msTNG+0.25,qfP100(:,k),'--x','color',cmap(k,:),...
    %         'DisplayName',ltag,'linewidth',1.5);
    %     %if k==1; hold on; end
    % end
...
%             'DisplayName',ltag,'linewidth',1.5);
    ...
    fname=sprintf('qf_comparison_ms_%s',ptag);
