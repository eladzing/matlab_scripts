...
for i=1:length(list);
    ...
    for b=2 ;%4];
        ...
        ros=smooth3(ro,smoothtype,smscale);
        clear pdm pgs ro
        ...
        Pr=ros.*tp.*f_pr;
        ekin(:,:,:,1)=0.5.*ros.*(Vxx.^2+Vyy.^2+Vzz.^2).*bmask;
        egrav(:,:,:,1)=pot.*ros.*bmask;
        eth(:,:,:,1)=ros.*tp.*f_th.*bmask;
        clear tp
        
        %qcube=cooling_cube(boxx)./ro;%.*bmask;  %% per gram
        qcube(:,:,:,1)=cooling_cube(boxx);%.*bmask;
        qcube=smooth3(qcube(:,:,:,1),smoothtype,smscale);
        ...
        for kk=1:2
            ...
            %  save(matname,normfac,qdiss,ekin,egrav,eth,qcube,pdv,...
