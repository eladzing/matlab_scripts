function hout=streamline_new(varargin)
...
%   $Revision: 1.9.4.4 $  $Date: 2005/04/28 19:57:06 $
  ...
  if ~isempty(vv)
    ...
    if size(vv,2)==3
      h = [h ; line('xdata', vv(:,1), 'ydata', vv(:,2), 'zdata', vv(:,3), ...
              'color', color_map(dist,:), 'parent', cax)];
    else
...
if nin==1  % streamline(xyz) or  streamline(xy) 
  verts = vargin{1};
  if ~iscell(verts)
    error(id('NonCellVertices'),'Stream vertices must be passed in as a cell array')
  end
