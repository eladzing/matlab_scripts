...
if ~skip2point
    ...
    qGroupMask=polyMask & galMask;
    qGroupInd=find(qGroupMask);
    
    %% create mass-matched 
    oGroupMask=galMask & ~qGroupMask ;
    oGroupInd=find(oGroupMask);
    
    matched=create_matched_sample(qGroupInd,oGroupInd,galMass,'range');
    
    groupMask=false(size(qGroupMask));
    groupMask(matched)=true;
end
...
for id=0:len-1
    ...
    if groupMask(id+1)
        ...
        if mod(cnt,50)==0 % print interim
            ...
            fname=sprintf('matchedGroup_history_z%s_%s_id%i',num2str(illustris.utils.get_zred(snap)),simDisplayName,id);
            fnameBird=sprintf('matchedGroup_history_birds_z%s_%s_id%i',num2str(illustris.utils.get_zred(snap)),simDisplayName,id);
...
if DRACOFLAG
    %global DEFAULT_MATFILE_DIR
    %global simDisplayName
    fname=sprintf('matchedGroup_history_z%s_%s',num2str(illustris.utils.get_zred(snap)),simDisplayName);
    fnameBird=sprintf('matchedGroup_history_birds_z%s_%s',num2str(illustris.utils.get_zred(snap)),simDisplayName);
    save([DEFAULT_MATFILE_DIR '/' fname],'galHistory','done','-v7.3')
    save([DEFAULT_MATFILE_DIR '/' fnameBird],'galBirdHistory','done','-v7.3')
    fprintf(' *** Result saved to: %s *** \n',[DEFAULT_MATFILE_DIR '/' fname]);
    fprintf(' *** Bird Result saved to: %s *** \n',[DEFAULT_MATFILE_DIR '/' fnameBird]);
