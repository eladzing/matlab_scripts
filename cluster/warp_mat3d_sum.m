function warped = warp_mat3d_sum(cube, transform_matrix)
...
[meshX, meshY, meshZ] = meshgrid(1:size(cube,1), 1:size(cube,2), 1:size(cube,3));%(size(cube,3)+1)/2);
...
XI = reshape(solutions(1,:), size(cube));
YI = reshape(solutions(2,:), size(cube));
ZI = reshape(solutions(3,:), size(cube));

%interpolate (bilinear interpolation)
warped = interp3(cube, XI, YI, ZI);

warped = reshape(sum(warped,3),[size(cube,1), size(cube,2)]);
