function res=galEvolutionMachineLight(gal,host,orb,varargin)

units;
timeunit=(Units.kpc/Units.km)/(Units.Gyr);
time=orb.t.*timeunit;
endIndex=length(orb.t);
% defaults
alf=1;
...
while i<=length(varargin)
    switch(lower(varargin{i}))
        case {'alf','alfa','rpsalpha','rpsalfa'}
            i=i+1;
            alf=varargin{i};
        case{'sfra','sfrafac'}
            i=i+1;
...
vsat2=orb.vx(indx).^2+orb.vy(indx).^2;
