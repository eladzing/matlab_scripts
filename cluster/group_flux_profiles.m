...
load mat_files/stk_fluxes_a1.mat
...
rp=0.001:0.001:2.6;
    ...
    for i=lst %%1:length(lst);
        ...
        %if i~=14
        normv=flux_stack{i,6};
        n=n+1;
        clname{n}=sprintf('CL%s',num2str(flist(i)));
        mf(end+1,:)=interp1(stack{i,2},flux_stack{i,2},rp,'linear').*1e9;   %%./(0.056.*(normv(1)./1e13).^0.15).*1e9;
        mf_in(end+1,:)=interp1(stack{i,2},flux_stack{i,3},rp,'linear').*1e9; 
        mf_out(end+1,:)=interp1(stack{i,2},flux_stack{i,4},rp,'linear').*1e9; 
        mf_st(end+1,:)=interp1(stack{i,2},flux_stack{i,5},rp,'linear').*1e9; 
...
for mi=1:4
    switch (mi)
        case 1
            mdf = mf;
            titletag='Mass Flux';tag = 'mflx';
        case 2
            mdf = mf_in;
            titletag='Mass Flux - Inflowing'; tag = 'mflx_in';
        case 3
            mdf = mf_out;
             titletag='Mass Flux - Outflowing';tag = 'mflx_out';
        case 4
            mdf = mf_st;
            titletag='Mass Flux - Strong (v_r < -0.5V_{vir}';tag = 'mflx_strong';
...
%          %saveas(gcf,sprintf('%s/%s_stack_profs',result_dir,'multi'),'eps');
          saveas(gcf,sprintf('%s/%s_tile_profs.png',result_dir,tag));
    end
end
