% filterString2Mask - Converts a filter string into a mask. If the filter 
% string is invalid, it is manipulated to form a valid string which is 
% returned in place of a mask.
...
while ~isempty(token) || ~isempty(fString)
	...
	elseif length(i)>1
		...
		if i==1 || i==length(token)
			...
			thisok = true;
			first = str2double(token(1:i-1));
			last = str2double(token(i+1:end));
			if first<minVal
				first = minVal;
