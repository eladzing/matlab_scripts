function result = MAKE_PROFILE_FROM_CUBE_AVG(data,weight)
% Returns an average profile for cartesian cubes of extensive parameters.  
...
result = zeros([1 257]);
count=ones(size(result));

data = double(data.*weight);
for i = 1:256
    ...
    count(i) = count(i)+ sum(weight(mask) .* (1 - upperFraction(mask))); 
    count(i+1) = count(i+1)+ sum(weight(mask) .* upperFraction(mask)); 
end


result = result(1:256)./count(1:256);