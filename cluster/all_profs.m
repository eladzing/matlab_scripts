...
if strcmp(plotflag,'plot')
    ...
    loglog(RP/r200,tcool,RP/r200,tcon,RP/r200,tdyn1);grid;xlim([RP(1) 3]);ylim([5e-3 7e3]) 
    set(gcf,'DefaultAxesColorOrder',[0 0 1;1 0 0;0 1 0])
    %xlabel('r/R_{vir}');
    ylabel(sprintf('t_{dyn},t_{cool},t_{con} [Gyr]'));
    title(sprintf( '%s Dynamical, Contraction and Cooling Time Profile',clustername));
    legend('t_{cool}','t_{con}', 't_{dyn}','Location','NorthWest');
    line([min(RP/r200) max(RP/r200)], [14 14],'Color','Black');
    %subplot(3,1,3);
    %loglog(RP/r200,tcon./tcool);grid;xlim([RP(1) 3]);ylim([5e-3 30])
    %xlabel('r/R_{vir}');ylabel(sprintf('t_{con}/t_{cool}'));
    ...
    set(gcf,'DefaultAxesColorOrder',[0 0 1;1 0 0;0 1 0])
    xlabel('r/R_{vir}');
    ylabel(sprintf('|dM_{gas}/dt|, |M_{gas}/t_{cool}| [M_{sun}/yr]'));
    title(sprintf( '%s Mass Flux Profile',clustername));
    legend('Mass flux', 'M_{gas}/t_{cool}','Location','NorthEast');
 
    %subplot(3,1,3)
    %loglog(RP/r200,abs(mdfull./Mdp));grid;xlim([RP(1) 3]);ylim([1e-2 1e2])
    %xlabel('r/R_{vir}');ylabel(sprintf('|dM_{gas}/dt|/|M_{gas}/t_{cool}|'));
