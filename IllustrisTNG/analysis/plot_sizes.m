...
cmdot=brewermap(2,'Set1');


figure('position',[186   516   766   649]);
ax0=axes; 
xlim(xl);ylim(yl);
...
ax1=axes;
contour(popCont1.xx,popCont1.yy,popCont1.popContour,'ShowText','off','LineColor','k','linewidth',1,...
    'LevelList',[8:15:98],'Fill','off','linestyle','-');
xlim(xl);ylim(yl);
...
ax2=axes;
contour(popCont2.xx,popCont2.yy,popCont2.popContour,'ShowText','off','LineColor','k','linewidth',1,...
    'LevelList',[8:15:98 ],'Fill','off','linestyle','-');
xlim(xl);ylim(yl);
...
% 
% for i=1:len
%  str=sprintf('$%4.1f$',10.^hostMedR.yMedian(i));
...
% end
% 
% h(1)=plot(xdata,y1data,'.','color',cmdot(1,:),'DisplayName','$r_\mathrm{star}$');
% h(2)=plot(xdata,y2data,'.','color',cmdot(2,:),'DisplayName','$r_\mathrm{1/2,gas}$');
% 
% 
% grid
% box on
...
% 
% ax1=axes;
% contour(popCont1.xx,popCont1.yy,popCont1.popContour,'ShowText','off','LineColor','k',...
%     'LevelList',[8:15:98],'Fill','off','linestyle','-');
% xlim(xl);ylim(yl);
% set(ax1,'position',get(ax0,'position'));
% 
...
% 
% ax2=axes;
% contour(popCont2.xx,popCont2.yy,popCont2.popContour,'ShowText','off','LineColor','k',...
%     'LevelList',[8:15:98 ],'Fill','off','linestyle','-');
% xlim(xl);ylim(yl);
% set(ax2,'position',get(ax0,'position'));
% 
...
% % %% normalized 
% % 
% % y1data=log10(rStar./hostRad);
% % y2data=log10(rhalfGas./hostRad);
% % 
% % 
% % filt=fspecial('disk',8);
% % 
...
% % % 
% % % 
% % % rsMed=mk_meanMedian_bin(xdata,y1data,'bins',9:0.5:12.5);
% % % rgMed=mk_meanMedian_bin(xdata,y2data,'bins',9:0.5:12.5);
% % % 
% % % figure
% % % 
% % % h(1)=plot(xdata,y1data,'b.','DisplayName','$r_\mathrm{star}$');
% % % hold on
% % % h(2)=plot(xdata,y2data,'r.','DisplayName','$r_\mathrm{1/2,gas}$');
% % % %errorbar(rsMed.xMedian,rsMed.yMedian,rsMed.yQuarts(1,:),rsMed.yQuarts(2,:))
% % % errorbar(rsMed.xMedian,rsMed.yMedian,...
% % %     abs(rsMed.yMedian-rsMed.yQuarts(1,:)),abs(rsMed.yMedian-rsMed.yQuarts(4,:)),...
% % %     'k','linewidth',2)
% % % 
% % % errorbar(rgMed.xMedian,rgMed.yMedian,...
% % %     abs(rgMed.yMedian-rgMed.yQuarts(1,:)),abs(rgMed.yMedian-rgMed.yQuarts(4,:)),...
% % %     'k','linewidth',2)
% % % 
% % 
