%% Dissipation Energy CalculattionCalculattion
...
ekin=0.5.*ro.*(Vxx.^2+Vyy.^2+Vzz.^2).*vol.*bmask;
egrav=ro.*(pdm+pgs).*vol.*bmask;
eth=ro.*tp.*f_th.*vol.*bmask;
clear pdm pgs ro tp 

qcube=cooling_cube(boxx).*vol;%.*bmask;
pdv=-1.*Pr.*divu.*fu.*vol.*bmask; % comes out positive for convergent flow 
...
% 
% qdiss(:,:,:,b)=deltaK+deltaP+pdv-qcube;
