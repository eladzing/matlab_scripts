...
for snap=[99 84 67 50 40 33]
    ...
    if readFlag
        ...
        fprintf('Reading in data snap %i \n',snap);
    ...
    fprintf('total Mask pool (no subflag & periodic) = %i , %f \n',sum(mask),sum(mask)./length(mask))
    
    mask0=  satMask & massMask & hostMask & distMask & gasMaskGal & dmMask & subMask;
    fprintf('total Mask pool (periodic = %i , %f \n',sum(mask0),sum(mask0)./length(mask0))
    
    
    mask1=  satMask & massMask & hostMask & distMask1 & gasMaskGal & dmMask;
    fprintf('total Mask pool (no periodic & no subflag)= %i , %f \n',sum(mask1),sum(mask1)./length(mask1))
    
    mask2=  satMask & massMask & hostMask & distMask1 & gasMaskGal & dmMask & subMask;
    fprintf('total Mask pool (no periodic w/ subflag)= %i , %f \n',sum(mask2),sum(mask2)./length(mask2))
    ...
    sidF=find(mask0)'-1; %  % subfind id's of objects in larger sample
    sidF1=find(mask2)'-1;
