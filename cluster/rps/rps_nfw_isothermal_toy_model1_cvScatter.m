...
%% build isothermal-nfw gas distribution
...
lStrip=zeros(length(rp),length(ms),length(cvfac)); %zeros(size(rp));
msN=zeros(length(rp),length(ms),length(cvfac));
% msat=zeros(length(xx),length(ms));
...
for j=1:length(ms);
    ...
    for k=1:length(cvfac)
        ...
        bf=(trapz(xx,xx.^2.*exp(2./as(k).*log(cs(k).*xx+1)./xx))).^-1;
        
        msat=bf.*cumtrapz( xx, xx.^2.*exp(2./as(k).*log(cs(k).*xx+1)./xx));
        
        rhs=(fg*(1+0)/fc/alfa).*(ms(j)./mc).^(2/3).*(ac(k)./as(k)).*bf.*exp(2/as(k).*log(1+cs(k).*ls)./ls); %%(ls.^2.*(cs(k).^-1+ls).^2)./nfwA(ls,cs(k)); % radius in satellite
        
        
        for i=1:length(rp)
            
            lhs=1./( rp(i).*(cc(k)^-1+rp(i)).^2);%./nfwA(rc,cc);
            
            ff=rhs./lhs;
            
            lStrip(i,j,k)=interp1(ff,ls,1,'PCHIP');
        ...
        msN(:,j,k)=interp1(xx,msat,lStrip(:,j,k),'PCHIP');  %nfwA(lStrip,cs(k))./nfwA(1,cs(k));
     end
end
