...
load([DEFAULT_MATFILE_DIR '/qFracFullOrb.mat'])
qFracFull=qFrac;
clear qFrac
load([DEFAULT_MATFILE_DIR '/qFracNew.mat'])
...
for i=1:7
    ...
    for j=1:7
        ...
        bCen=qFrac(i,j).bCen;
        bCenF=qFracFull(i,j).bCen;
        
        qf=qFrac(i,j).qfProj./qFrac(i,j).cntProj;
        qfF=qFracFull(i,j).qfProj./qFracFull(i,j).cntProj;
        
        plot(bCen,qf,st,'color',col,...
            'linewidth',lw,'Displayname',bTag);
        if j==1; hold on;  end
        h(j)=plot(bCenF,qfF,stF,'color',col,...
            'linewidth',lwF,'Displayname',bTag);
        
        % h(2)=plot(bCen,qf2./cnt2,'rx-',...
        %        'linewidth',lw,'Displayname','projected');
    end
    
    h(end+1)=plot(0.68.*Wetzel12_fig15(2,:),Wetzel12_fig15(1,:),'^k',...
        'DisplayName','Wetzel+12','markersize',10,'markerFaceColor','k');
...
for i=1:7
    ...
    for j=1:7
        ...
        bCenF=qFracFull(j,i).bCen;
        
        qfF=qFracFull(j,i).qfProj./qFracFull(j,i).cntProj;
