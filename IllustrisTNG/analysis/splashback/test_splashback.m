function [res]=test_splashback(galID,varargin)
...
while(ii<=length(varargin))
    switch(lower(varargin{ii}))
        ...
        case {'verbose','v'}
            verboseFlag=true;
        otherwise
            error('TEST_SPLASHBACK - Illegal argument: %s',varargin{ii});
    end
    ii=ii+1;
end

% get the tree
treeFields={'SnapNum','SubfindID','SubhaloPos','SubhaloHalfmassRadType',...
    'SubhaloID','SubhaloMassInRadType','SubhaloSFRinRad','Group_M_Crit200',...
...
resBird.birdXlim=tlim;
resBird.birdYlim=nlim;
...
inGal2.bird=zeros(blen,blen,len);
...
for i=1:lastInd
    ...
    for k=1:2
        ...
        if any(mask)
            ...
            if fullSnapMask(snap+1)
                tc=tcool(mask);
                enrgyDiss=gas.EnergyDissipation(mask).*illUnits.EnergyDissipationUnit; %in units of 10^45 ergs/yr
            ...
            if k==2
                ...
                if fullSnapMask(snap+1)
                    ediss=gas.EnergyDissipation(maskt).*illUnits.EnergyDissipationUnit;
                ...
                % total Bird
                if birdFlag
                    [bird, ~,~,~]= histogram2d(log10(tmp),log10(nDens),mm,...
                        'xxlim',tlim,'yylim',nlim,'len',blen);
                    inGal2.bird(:,:,i)=squeeze(bird(:,:,1));
                end
            end
    ...
    for k=1:2
        ...
        if any(mask)
            ...
            if fullSnapMask(snap+1)
                tc=tcool(mask);
                enrgyDiss=gas.EnergyDissipation(mask).*illUnits.EnergyDissipationUnit;
                mach=gas.Machnumber(mask);
...
resBird.inGal=inGal2;
resBird.inCGM=inCGM2;
resBird.inSub=inSub2;
