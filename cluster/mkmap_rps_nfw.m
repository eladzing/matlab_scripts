function mkmap_rps_nfw(boxx,varargin)
...
switch stripmodel
    ...
    case {'mv_strip','mv_strip_inv'}
        ...
        cvSat=cvir_Mvir(mv,zred);
        as=nfwA(1,cvSat);
            
            
        pval1=(fgas/alfa)*(G*Ms/Mpc/km^2)/(4*pi)*(mv^2/rv^4);
        pval=pval1.*nfwA(ls,cvSat)./(as.*ls.*(cvSat.^-1+ls)).^2;
...
if typeflag
    ...
    switch lower(type)
        case('mv_strip') % show stripping radius for a given sat mass
            lstrip=interp1(pval,ls,rpscube);
            cube=nfwA(lstrip,cvSat)./nfwA(1,cvSat);
