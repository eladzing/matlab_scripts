...
Vxx = squeeze(sum(Vxx(:,:,127:130),3));
Vyy = squeeze(sum(Vyy(:,:,127:130),3));
Vzz = squeeze(sum(Vzz(:,:,127:130),3));
...
quiver(Vyy(1:DILUTE:end,1:DILUTE:end),Vxx(1:DILUTE:end,1:DILUTE:end),2.5)
...
b(b>1) = 1;
imshow(b);
imwrite(b, sprintf('entropy_quiver_fix_1_%d.jpg',MPSec));
