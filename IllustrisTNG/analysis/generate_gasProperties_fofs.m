...
%         
        ...
        if any(mask)
            ...
            if sum(tcMask)>0
                ...
                %massHist.inR200.tcool(:,id+1)=xx;
                %massHist.inR200.tcoolMass(:,id+1)=massDist;
                
                gasPropsFofs.inR200.meanTcMW(id+1)=sum(mm(tcMask).*tc(tcMask))/sum(mm(tcMask));
                gasPropsFofs.inR200.stdTcMW(id+1)=calc_standardDev(tc(tcMask),mm(tcMask));
                gasPropsFofs.inR200.medianTc(id+1)=10.^mus(3);
                gasPropsFofs.inR200.quantTc(:,id+1)=10.^mus([1 2 4 5]);
                ...
                for i=1:ll
                    ...
                    gasPropsFofs.inR200.massBinTc(i,id+1)=sum(massDist(msk));
                end
            end
            % temp
            [xx, massDist, mus]=mk_mass_histogram(log10(tmp),mm,qus);
            ...
            gasPropsFofs.inR200.sfrMass(id+1)=sum(mass(distMask & sfMask));
        ...
        if any(mask)
            ...
            if sum(tcMask)>0
                ...
                %massHist.inTot.tcool(:,id+1)=xx;
                %massHist.inTot.tcoolMass(:,id+1)=massDist;
                
                gasPropsFofs.inTot.meanTcMW(id+1)=sum(mm(tcMask).*tc(tcMask))/sum(mm(tcMask));
                gasPropsFofs.inTot.stdTcMW(id+1)=calc_standardDev(tc(tcMask),mm(tcMask));
                gasPropsFofs.inTot.medianTc(id+1)=10.^mus(3);
                gasPropsFofs.inTot.quantTc(:,id+1)=10.^mus([1 2 4 5]);
                ...
                for i=1:ll
                    ...
                    gasPropsFofs.inTot.massBinTc(i,id+1)=sum(massDist(msk));
                end
            end
            ...
            gasPropsFofs.inTot.sfrMass(id+1)=sum(mass(distMask & sfMask));
