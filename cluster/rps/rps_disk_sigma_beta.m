...
mstrip=zeros(length(beta),length(ls));
pval=zeros(length(beta),length(ls));
...
for i=1:length(beta)
    ...
    for j=1:length(se)
        ...
        pval(i,j)=etap.*rps_factor('alpha',alfa,'fc',fc,'fd',fd,'beta',beta(i),...
            'Mc',Mc,'sigma',se(j),'type','sigma_eff');
