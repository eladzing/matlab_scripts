...
for id=ids
    ...
    for k=1:4
        ...
        gasMass.tot(k,cnt)=sum(mass(radMask(k,:) & ~sfMask ));
        gasMass.sf(k,cnt)=sum(mass(radMask(k,:) & sfMask ));
        
        gasMass.count.cold(k,cnt)=sum(coldMask & radMask(k,:));
        gasMass.count.cool(k,cnt)=sum(coolMask & radMask(k,:));
        gasMass.count.warm(k,cnt)=sum(warmMask & radMask(k,:));
        gasMass.count.hot(k,cnt)=sum(hotMask & radMask(k,:));
        gasMass.count.tot(k,cnt)=sum(radMask(k,:) & ~sfMask );
        gasMass.count.sf(k,cnt)=sum(radMask(k,:) & sfMask );
    end
