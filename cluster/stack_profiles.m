...
%%plot stacks
    ...
    semilogx(rp,mdf','linewidth',1);grid;xlim([1e-2 3]);ylim([-0.6 0.2]);
    xlabel('r/R_{vir}','Fontsize',14);%,'Interpreter','latex');  
    ylabel(' flux/M_{vir} [Gyr^{-1}] ','Fontsize',14);% / M_{vir} [Gyr^{-1}] $}','Fontsize',14,'Interpreter','latex');
    title(sprintf('%s Clusters Mass Flux',titletag),'Fontsize',14);
    line([1e-3 1e1],[0 0],'Color','k');
    %legend(clname,'location','bestoutside');
    set(gca,'Fontsize',14,'linewidth',1);
    
    %laprint(gcf,'w08','options','factory');
       
     if strcmp(pflag,'print')
        saveas(gcf,sprintf('%s/%s_stack_flux_prof_w08.png',result_dir,printag));
    end
%     
%     figure;
...
%     legend(clname,'location','bestoutside');
%     set(gca,'Fontsize',12,'Interpreter','latex');  
%     if strcmp(pflag,'print')
