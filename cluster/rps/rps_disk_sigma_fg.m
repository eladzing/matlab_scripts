...
mstrip=zeros(length(ls),length(ffd));
pval=zeros(length(ls),length(ffd));
...
for i=1:length(ls)
    for j=1:length(ffd)
        ...
        pval(i,j)=etap.*rps_factor('alpha',alfa,'fc',fc,'fd',ffd(j),'Mc',Mc,...
            'sigma',se(i),'type','sigma_eff');
           
        %sigmae(i,j)=sigma_effective(mm(i),rd(j));
        if (pval(i,j)<=fgmax)
            mstrip(i,j)=interp1(fg,mf,pval(i,j));
        elseif (pval(i,j)>fgmax)
            mstrip(i,j)=0;
        elseif(pval<fg(end))
            mstrip(i,j)=1;
        end
