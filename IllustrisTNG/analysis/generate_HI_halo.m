...
for fld=compNames
    ...
    for param=paramNames
        for prop=propNames
            ...
            switch param
                case {"H2", "HI"}
                    PropStruct.(fld).(pname)=zeros(3,len);
                otherwise
        ...
        for fld=compNames
            ...
            if any(mask)
				...
				for k=1:3
					...
					param='HI'
					PropStruct.(fld).(strcat(fld,param,'mass'))(k,id+1)=sum(gas.mHi(k,mask))
					param='H2'
					PropStruct.(fld).(strcat(fld,param,'mass'))(k,id+1)=sum(gas.mH2(k,mask))
				end
				
				
				
				PropStruct.(fld).(strcat(fld, 'GasMass'))(id+1)=sum(mm); % All gas Mass
