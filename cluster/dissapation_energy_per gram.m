...
innermask(65:192,65:192,65:192)=false;qdiss(:,:,:,b)=deltaK+deltaP+pdv-qcube;
...
ekin=0.5.*(Vxx.^2+Vyy.^2+Vzz.^2).*bmask;
egrav=(pdm+pgs).*bmask;
eth=tp.*f_th.*bmask;
clear pdm pgs ro tp 

qcube=cooling_cube(boxx)./ro;%.*bmask;
pdv=-1.*Pr.*divu.*fu.*bmask./ro; % comes out positive for convergent flow 
