...
figure
for i=1:16
    semilogx(mv_a1(i).*[1 1],edgerange_a1(i,:),'b','linewidth',3.5)
    if i==2
        hold on
    end
    semilogx(mv_a06(i).*[1 1],edgerange_a06(i,:),'r','linewidth',3.5)
    
end
%semilogx(mv1,yy1,'ob','markersize',10.5,'markerfacecolor','auto')
%plot(mv_a1,edgemean_a1,'--')
%semilogx(mv6,yy6,'or','markersize',10.5,'markerfacecolor','auto')
hold off
hl=legend('$z=0$','$z=0.6$');
set(hl,'interpreter','latex','fontsize',12)
xlabelmine('$M_{vir}\,[10^{14}\mathrm{M_\odot}]$')
...
name=sprintf('%s/cledge_vs_mv_a06',printoutdir)
%exportfig(gcf,sprintf('%s.png',name),'format','png');
%exportfig(gcf,sprintf('%s.eps',name));
