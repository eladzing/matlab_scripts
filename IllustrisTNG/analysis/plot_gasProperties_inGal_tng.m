...
if ssfrFlag
    ...
    % tcool
    plot_massSsfr2('tree',xdata,ydata,log10(tc(galaxyMask)),tre,tcLab,cmap,popCont,hostMed,[-2.5 1.5]);
    fname=sprintf('massSsfr_%s_%s','tcool',nameTag);
    if  printFlag; printout_fig(gcf,fname,'fig','v'); end
    
    
     % Entropy
    plot_massSsfr2('tree',xdata,ydata,log10(gasEnt(galaxyMask)),tre,entLab,cmap,popCont,hostMed,[-3 3]);
    fname=sprintf('massSsfr_%s_%s','tcool',nameTag);
    if  printFlag; printout_fig(gcf,fname,'fig','v'); end
