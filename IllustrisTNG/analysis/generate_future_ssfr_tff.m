...
sfrStruct.inGal.fMs =zeros(5,len);
sfrStruct.inGal.fMs10 =zeros(5,len);
...
ts=[0.1 0.5 1.0 1.33 1.6 2.0 5.0]; %in Gyr
...
%                 
                ...
                if sum(tcMask)>0
                    for k=1:length(ts)
                        sfrStruct.(fld).fMs(k,id+1)= sum(mm(tcMask & tf<(ts(k))));
                    end
                end
                
                if sum(tcMask10)>0
                    for k=1:length(ts)
                        sfrStruct.(fld).fMs10(k,id+1)= sum(mm(tcMask10 & tf<(ts(k))));
                    end
                end
...
if DRACOFLAG
    ...
    fname=sprintf('futureSFR_tff_snp%s_%s.mat',num2str(snap),simDisplayName);
